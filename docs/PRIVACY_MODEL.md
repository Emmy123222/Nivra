...
| `merchantCommitment` (`persistentHash(DOMAIN_MERCHANT, merchantSecret, merchantNonce)`) | PUBLIC | Needed on-chain so `cancelInvoice` can check authorization without ever learning `merchantSecret`; reveals nothing about the merchant's real-world identity |
...
| The transient Zswap shielded coin moved during settlement | PRIVATE amount and owner; but the **specific coin object's commitment linkage is disclosed** | Calling `receiveShielded`/`sendShielded` requires explicit `disclose()` because it links the receive/spend event to that coin's commitment. Amount and token color still never appear in contract ledger state. |
